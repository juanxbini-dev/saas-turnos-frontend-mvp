...
import type { CampaniaMetricasTotales } from '../../types/campanias.types';
...
export function CampaniaMetricas() {
  ...
  const { data, loading, error, revalidate } = useFetchVigente(
    buildKey(ENTITIES.CAMPANIAS, 'recencia', 'metricas', periodo.fecha_desde),
    () => campaniasService.getMetricas('recencia', periodo),
    { ttl: TTL.MEDIUM }
  );
  ...
  const serie = useMemo(() => {
    ...
    return diasDelPeriodo(periodo).map((fecha) => {
      ...
      return { fecha, enviados: punto?.enviados ?? 0, reservas: punto?.conversiones ?? 0 };
    });
  }, [data, periodo]);
        ...
        <>
          ...
          <div className="mt-5">
            {cargando ? (
              ...
              <div className="h-64" role="img" aria-label="Mensajes enviados y reservas por día">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={serie} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                    ...
                    <Tooltip
                      formatter={(valor, nombre) => [Number(valor) || 0, nombre === 'enviados' ? 'Enviados' : 'Reservas']}
                      labelFormatter={(f) => {
                    ...
                    <Legend
                      formatter={(valor: string) => (
                        <span className="text-xs text-gray-600">{valor === 'enviados' ? 'Enviados' : 'Reservas'}</span>
                      )}
                    />
